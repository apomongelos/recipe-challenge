"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UserSignUpInput = void 0;
const type_graphql_1 = require("type-graphql");
const class_validator_1 = require("class-validator");
const IsEmailAlreadyExist_1 = require("./IsEmailAlreadyExist");
let UserSignUpInput = class UserSignUpInput {
};
__decorate([
    type_graphql_1.Field(),
    class_validator_1.Length(1, 255, { message: 'Nombre debe ser mayor a un caracter.' }),
    __metadata("design:type", String)
], UserSignUpInput.prototype, "name", void 0);
__decorate([
    type_graphql_1.Field(),
    class_validator_1.IsEmail({}, { message: 'Debe proporcionar un email.' }),
    IsEmailAlreadyExist_1.IsEmailAlreadyExist({ message: 'Email ya esta en uso' }),
    __metadata("design:type", String)
], UserSignUpInput.prototype, "email", void 0);
__decorate([
    type_graphql_1.Field(),
    class_validator_1.Length(5, 255, {
        message: 'Contraseña corta, debe ser mayor a 5 caracteres',
    }),
    class_validator_1.Matches(/^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])[a-zA-Z0-9]+$/, {
        message: 'Contraseña insegura, incluya mayusculas, minusculas y numeros',
    }),
    __metadata("design:type", String)
], UserSignUpInput.prototype, "password", void 0);
UserSignUpInput = __decorate([
    type_graphql_1.InputType()
], UserSignUpInput);
exports.UserSignUpInput = UserSignUpInput;
